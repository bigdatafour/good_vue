    ...
    <div class="nav-item">
      <ul>
        <li v-for="(item,index) in nav" :key="index">
          <span @click="cityAdvertisement(item)">
            <a href="#">{{item}}</a>
          </span>
        </li>
      </ul>
      ...
      <div class="detail-item-panel panel-1" :duration="{ enter: 100, leave: 100 }" v-show="panel1" @mouseenter="showDetail(1)" ref="itemPanel1" @mouseleave="hideDetail(1)">
        ...
        <ul>
          <li v-for="(items, index) in panelData1.classNav" :key="index" class="detail-item-row">
            ...
            <!--<router-link to="/goodsList" v-for="(item, subIndex) in items.tags" :key="subIndex" @click="onclick(item)">
              <span class="detail-item">{{item}}</span>
            </router-link>-->
            <a v-for="(item, subIndex) in items.tags" @click="onclick(item)">
              <span class="detail-item">{{item}}</span>
            </a>
          </li>
        </ul>
      ...
      <div class="detail-item-panel panel-2" :duration="{ enter: 100, leave: 100 }" v-show="panel2" @mouseenter="showDetail(2)" ref="itemPanel2" @mouseleave="hideDetail(2)">
        ...
        <ul>
          <li v-for="(items, index) in panelData2.classNav" :key="index" class="detail-item-row">
            ...
            <!--<router-link to="/Login" v-for="(item, subIndex) in items.tags" :key="subIndex">
              <span class="detail-item">{{item}}</span>
            </router-link>-->
            <a v-for="(item, subIndex) in items.tags" @click="onclick(item)">
              <span class="detail-item">{{item}}</span>
    ...
    export default {
        ...
        methods: {
            ...
            onclick (item) {
                axios({
                    method:"post",
                    url:"http://localhost:8082/shopOnline/goodsClick",
                    headers: {
            ...
            cityAdvertisement (item) {
                if(sessionStorage.getItem("user_name") != null & sessionStorage.getItem("cityName") != null) {
                    axios({
                        ...
                        data: {"city_name": sessionStorage.getItem("cityName"), "ad": item}
                    }).then((res) => {
                        console.log(res);
                        this.$router.push("/goodsList")
                    });
                ...
                //console.log(this.sessionStorage.getItem("cityName"))
            }
        },
        mounted () {
